...
# FIX IMPORT: Thêm root project vào sys.path
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
PROJECT_ROOT = os.path.dirname(SCRIPT_DIR)
sys.path.append(PROJECT_ROOT)

# BÂY GIỜ IMPORT AN TOÀN
from app.connect_db.mongo_client import orders
from bson import ObjectId

load_dotenv()

OUT_PATH = os.getenv("ITEM_SIM_PATH", "data/item_similarity.json")
...
def extract_purchase_history():
    ...
    total = 0
    for order in orders.find({}, {"userId": 1, "products": 1}):
        total += 1
        ...
        for p in order.get("products", []):
            pid = p.get("productId")
            if pid:
                rows.append({"user_id": user_id, "item_id": str(pid)})
...
def compute_similarity(mat, items, top_k=50):
    ...
    for i, item_id in enumerate(items):
        scores = [(items[j], float(sim[i][j])) for j in range(len(items)) if j != i and sim[i][j] > 0.01]
...
def main():
    ...
    if df.empty:
        ...
        return
    print(f"Trích xuất: {len(df)} lượt | {df['user_id'].nunique()} người | {df['item_id'].nunique()} sách")
